...
## Шаг 7
Выполним прогрессивное выравнивание генома E. coli X и референсного штамма с помощью Mauve для выявления уникальных вставок, включая гены шига-токсина.

Запуск Mauve из графического интерфейса выполнялся по следующему алгоритму:
1. File → Align with progressiveMauve... → Add sequences  
2. Выбрать: reference.fasta (55989.fasta) и scaffolds.gbk (из шага 5)  
3. Задать имя выходного файла (например, alignment.xmfa) и запустить alignment  

После завершения выравнивания используем Sequence Navigator (иконка бинокля):  
1. В левом поле выбрать Product  
2. В правом поле ввести `shiga` (или `toxin`)

Найденные гены шига-токсина:  

| Gene name | Start (bp) | End (bp) | Length (bp) | Comment | Neighbouring phage genes (coordinates) |
|-----------|------------|----------|-------------|---------|----------------------------------------|
| stxA | 3,947,959 | 3,948,918 | 960 | Subunit A (toxic / catalytic activity) | Phage DNA adenine methylase (1,118,499–1,118,651) |
| stxB | 3,948,930 | 3,949,199 | 270 | Subunit B (cell binding / receptor recognition) | Unknown protein encoded within prophage CP-933V (1,120,760–1,120,867) |


## Шаг 8
Используя тот же файл выравнивания alignment.xmfa в Mauve, исследуем сегмент, содержащий найденные гены шига-токсина.

- Приближаем (zoom) область вокруг генов токсина.
- Наводим курсор на соседние гены и читаем их аннотацию (поле _Product_).

На основе аннотации соседних генов определяем происхождение генов токсина:
- [ЗДЕСЬ ДОБАВИТЬ ДАННЫЕ, например: «Гены расположены рядом с генами бактериофагов (интегразы, транспозазы), что указывает на горизонтальный перенос через профаг»]
...
| Локация (координаты) | Длина (bp) | Примечание |
|----------------------|------------|-----------------------------|
| 16,690 – 17,550 | 861 | Предположительный ген β-лактамазы |
| 20,372 – 21,247 | 876 | Предположительный ген β-лактамазы |
| 1,458,526 – 1,459,386 | 861 | Предположительный ген β-лактамазы |
| 4,703,556 – 4,704,689 | 1,134 | Предположительный ген β-лактамазы |

## Шаг 10. Механизм устойчивости к антибиотикам

Снова открываем выравнивание Mauve (`alignment.xmfa`) и с помощью Sequence Navigator (поиск по Product) находим гены β-лактамаз (например, `bla`, `beta-lactamase`, `CTX-M`, `TEM` и т.д.).

| Тип элемента | Координаты | Длина (bp) | Описание |
|--------------|------------|------------|-----------|
| Mobile element protein | 18,854 – 20,116 | 1,263 | Белок мобильного элемента (транспозон/интегрон) |
| Phage integrase (site-specific serine recombinase) | 1,459,569 – 1,459,820 | 252 | Фаговая интеграза — фермент, встраивающий ДНК в хромосому |
| Mobile element protein | 21886 – 24435 | — | Белок мобильного элемента (транспозон/интегрон) |

Механизм действия: β-лактамаза гидролизует β-лактамное кольцо антибиотиков (пенициллинов, цефалоспоринов), делая их неактивными.
